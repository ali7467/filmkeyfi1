    ...
    <div>
      ...
      <div className="grid sm:grid-cols-[240px_1fr] gap-4">
        <div className="space-y-2 overflow-y-auto max-h-[60vh]">
          {rooms.map((r) => <button key={r.id} onClick={() => setActive(r)} className={`w-full text-left p-3 rounded-lg border ${active?.id === r.id ? 'border-primary bg-primary/10' : 'border-border bg-card'}`}><p className="text-sm font-medium truncate">{r.name}</p><p className="text-xs text-muted-foreground">{r.status}</p></button>)}
        </div>
